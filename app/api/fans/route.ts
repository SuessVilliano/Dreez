...
const defaultQuestion='What should Dreez make next?';
...
export async function POST(req:Request){
 try{
  ...
  if(type==='vote'){
   ...
   const track=clean(b.track,160),artist=clean(b.artist,160),note=clean(b.note,800);if(!track)return json({error:'Tell Dreez what track you want to hear.'},400);address=b.email?email(b.email):'';payload={track,artist,note};
  }else if(type==='blend'){
