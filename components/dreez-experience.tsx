...
export default function DreezExperience() {
  ...
  useEffect(() => {
    ...
    const onMouseOut = (event: MouseEvent) => { if (event.clientY <= 0 && !event.relatedTarget) openExit(); };
    document.addEventListener("mouseout", onMouseOut);
    const mobileTimer = window.setTimeout(() => { if (window.innerWidth < 768) openExit(); }, 45000);
    return () => { document.removeEventListener("mouseout", onMouseOut); window.clearTimeout(mobileTimer); };
  }, []);

  const toggleTheme = () => {
    const next: Theme = theme === "dark" ? "light" : "dark";
    setTheme(next); document.documentElement.dataset.theme = next; window.localStorage.setItem("dreez-theme", next);
  };

  async function ask(text: string) {
    const cleaned = text.trim(); if (!cleaned || sending) return;
  ...
  function submitChat(event: FormEvent<HTMLFormElement>) { event.preventDefault(); void ask(input); }
